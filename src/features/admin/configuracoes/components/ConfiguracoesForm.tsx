...
export function ConfiguracoesForm({ estabelecimento }: { estabelecimento: Estabelecimento | null }) {
  ...
  const [corTema, setCorTema] = useState(estabelecimento?.cor_tema ?? '#0f2647');
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-card border border-border rounded-2xl p-6 space-y-5 text-foreground shadow-sm">
      <div className="flex items-center justify-between border-b border-border-light pb-4">
        <h2 className="text-base font-semibold text-foreground">Dados do estabelecimento</h2>
        <span className="text-xs text-muted bg-background-tertiary px-2.5 py-1 rounded-md font-mono">
      ...
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-xl bg-background-tertiary border border-border overflow-hidden flex items-center justify-center shrink-0">
          {logoPreview ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={logoPreview} alt="Logo" className="w-full h-full object-cover" />
          ) : (
            <ImagePlus size={22} className="text-muted-subtle" />
          )}
        </div>
        <div>
          <button
            ...
            className="text-sm font-medium text-primary hover:text-primary-hover hover:underline disabled:opacity-60 transition-colors"
          >
            {uploadingLogo ? 'Enviando…' : 'Trocar logo'}
          </button>
          <p className="text-xs text-muted-subtle mt-0.5">PNG ou JPG, quadrada de preferência</p>
        </div>
        <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleLogoChange} />
      </div>

      <div>
        <label className="block text-xs font-medium text-foreground mb-1.5">Nome do estabelecimento</label>
        <input
      ...
      <div>
        <label className="block text-xs font-medium text-foreground mb-1.5">Cor do tema</label>
        <div className="flex items-center gap-3">
          <input
            type="color"
            value={corTema}
            onChange={(e) => setCorTema(e.target.value)}
            className="w-10 h-10 rounded-lg border border-border cursor-pointer bg-background-secondary p-1"
          />
          <input
            type="text"
            className="input-base w-36 font-mono py-2 text-sm"
            value={corTema}
            onChange={(e) => setCorTema(e.target.value)}
          />
        </div>
