...
export type RegraEmissao = 'apenas_compra' | 'presenca_minima';
...
export interface EventoConcluido {
  ...
  // Novos campos de configuração de certificado
  emiteCertificado: boolean;
  cargaHoraria: number;
  regraEmissao: RegraEmissao;
  percentualPresenca: number; // Ex: 75 para 75%
  diasLiberacao: number;      // 0 = no mesmo dia do fim do evento
}
